...
\definition Также информацией Фишера называется
\begin{equation*}
...
\begin{equation*}
    \begin{aligned}
        ...
        % \frac{\partial\ln \mathcal{L}}{\partial\theta}&=\sum_i\frac{\partial\ln f_i}{\partial\theta}=0\Longrightarrow\widehat{\theta}=T(X_1,\ldots,X_n)
    \end{aligned}
\end{equation*}
...
\definition Считаем, что выполняются все условия гладкости и регулярности
\begin{equation*}
...
\definition $\widehat{\theta}$ асимптотически нормальна, если $\exists\sigma^2(\theta)$, что 
\begin{equation*}
    \sqrt{\pi}(\widehat{\theta}-\theta)\overset{d}{\longrightarrow} N(0,\sigma^2(\theta))
\end{equation*}

\definition Если $\widehat{\theta}$ — асимптотически нормальная, то $\frac{\sigma^2(\theta)}{n}$ называется асимптотической дисперсией

\definition Оценка $\widehat{\theta}$ — асимптотически эффективна, если ее асимптотическая дисперсия 
\begin{equation*}
...
\begin{itemize}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    \item \definition Если $\widehat{\theta}$ — асимптотически нормальна, то $\displaystyle\frac{\sigma^2}{n}=\text{AsVar}(\widehat{\theta})$ называется \textit{асимптотической дисперсией}
\end{itemize}
...
\definition Интервал $(T_1;T_2)$ называется \textit{доверительным интревалом} для параметра $\theta$ уровня доверия $1-\alpha$, если $\forall\theta\in\Theta$:
\begin{equation*}
...
\definition Статистическая гипотеза — утверждение о виде/свойствах распределения выборки $X=(X_1,\ldots,X_n)\sim F(x)$, где величины независимые и одинаково распределенные

\definition Статистический критерий — правило, позволяющее принять или отвергнуть проверяемую гипотезу
...
\definition Если $\Theta_0$ состоит из одной точки, то гипотеза $H_0$ называется \textit{простой}
...
\definition Функцией мощности критерия $\mathcal{X}_1$ называется
\begin{equation*}
...
\definition Мощностью критерия называется $1-\mathbb{P}(\text{ошибка 2 рода})$
