...
\definition Пусть $\xi:\Omega\rightarrow\mathbb{R}$ — $\fk$-измеримая функция. Функция $F_\xi:\mathbb{R}\longrightarrow[0;1]\ \forall x$ равная
$$\mathbb{P}(\{\omega\in\Omega:\xi(\omega)\leqslant x\}),x\in\mathbb{R}$$
...
Если функция $F:\mathbb{R}\longrightarrow[0;1]$ удовлетворяет данным свойствам, то она является функцией распределения некоторой случайной величины, то есть найдётся вероятностное пространство $(\Omega,\fk,\mathbb{P})$ и определённая на нём случайная величина $\xi$ такая, что $$F(x)=F_{\xi}(x)\ \forall x\in\mathbb{R}$$



\subsection{Дайте определение функции распределения $f_X (x)$ случайной величины $X$. Укажите необходимые и достаточные условия для того, чтобы функция была функцией плотности некоторой случайной величины}
...
Если некая функция $g(x)$ удовлетворяет следующим свойствам, то она является
функцией плотности распределения некоторой случайной величины
\begin{enumerate}
...
\definition \textit{Математическим ожиданием} $\matwait{\xi}$ случайной величины $\xi$ с \textbf{дискретным распределением} называется число 
\begin{equation*}
    \matwait{\xi}=\sum_{\omega\in\Omega}\xi(\omega)\prob{\omega}\footnote[1]{В учебнике Черновой дано такое выражение: $\matwait{\xi}=\sum_k a_k p_k=\sum_{k} a_k \prob{\xi=a_k}$, что, в приницпе, эквивалентно},
\end{equation*}
если данный ряд сходится \textit{абсолютно}

\definition \textit{Математическим ожиданием} $\matwait{\xi}$ случайной величины $\xi$ с \textbf{абсолютно непрерывным} распределением с плотностью распределения $f_{\xi}(x)$ называется число 
\begin{equation*}
    \matwait{\xi}=\int\limits_{-\infty}^{+\infty}xf_{\xi}(x)\d{x},
\end{equation*}
если этот интеграл сходится \textit{абсолютно}

По свойствам математического ожидания: $\matwait{\alpha X+\beta Y}=\alpha\matwait{X}+\beta\matwait{Y}$
...
\definition Дисперсией случайной величины $\xi$ называется число
\begin{equation*}
    \dispersia{\xi}=\matwait{(\xi-\matwait{\xi})^2}
\end{equation*}

По свойствам дисперсии: $\dispersia{\alpha X+\beta}=\alpha^2\dispersia{X}$
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Пуассоновское} — $Pois(\lambda)$
    \begin{itemize}
        \item $\matwait{\xi}=\lambda$
        \item $\dispersia{\xi}=\lambda$
        \item $\prob{\xi=k}=\displaystyle\frac{\lambda^k}{k!}e^{-\lambda}$, где $k=0,1,\ldots$
    \end{itemize}
    \item \textbf{Геометрическое} ($\xi$ — номер первого успешного испытания)
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{cases}$
    \end{itemize}
    \item \textbf{Экспоненциальное} с параметром $\lambda$
    \begin{itemize}
...
\definition $\forall x_1,\ldots,x_n\in\mathbb{R}^n$ независимы, если
\begin{equation*}
...
Если некоторая функция $g(x)$ удовлетворяет данным свойствам, то она является совместной функцией плотности распределения для некоторой пары случайных величин
