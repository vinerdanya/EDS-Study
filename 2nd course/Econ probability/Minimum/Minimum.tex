...
\begin{equation*}
    F_X(x):=\int\limits_{-\infty}^{+\infty}f_X(t)\d{t},x\in\mathbb{R},
\end{equation*}
...
\begin{itemize}
    ...
    \item $F_{X,Y}(-1;0)=\twoprob{X=-1}{\cap}{Y=-1}+\twoprob{X=-1}{\cap}{Y=0}=0.2+0.1=0.3$
    ...
    \item $\begin{tikzpicture}
        ...
        \draw[black, ->, thick] (0,0) -- (0,2) node [above left] {$F(x)$};
...
\subsection{[TBA] Пусть задана таблица совместного распределения случайных величин $X$ и $Y$}
\begin{equation*}
    \begin{tabular}{cccc}
        \hline
        &$Y=-1$&$Y=0$&$Y=1$\\
