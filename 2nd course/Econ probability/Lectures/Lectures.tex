...
\definition Ковариацией двух случайных величин называют 
\begin{equation*}
...
\definition $\xi^*=\displaystyle\frac{\xi-\matwait{\xi}}{\sigma_\xi}$, где $\sigma_\xi=\sqrt{\dispersia{\xi}}$

\comment $\matwait{\xi^*}=0,\dispersia{\xi^*}=1$

\definition $corr(\xi,\eta)=cov(\xi^*,\eta^*)$

\definition $corr(\xi,\eta)=\displaystyle\frac{cov(\xi,\eta)}{\sigma_\xi \sigma_\eta}$
...
\definition Пусть $\{\xi_n\}_{1}^{\infty}$ на $(\Omega, \fk, \mathbb{P})$. Тогда, $\{\xi_n\}$ сходится почти наверное (сходимость с вероятностью 1) к $\xi:\Omega\to\mathbb{R}$, если 
\begin{equation*}
    \prob{\omega:\xi_n(\omega)\to\xi(\omega)}=0
\end{equation*}

\definition $\{\xi_n\}$ сходится в среднем порядка $s$ к $\xi$, если 
\begin{equation*}
    \matwait{|\xi_n-\xi|^s}\underset{n\to\infty}{\to}0
\end{equation*}

\definition $\{\xi_n\}$ сходится по вероятности к $\xi$, если 
\begin{equation*}
...
\definition $\{\xi_n\}$ сходится по распределению к $\xi$, если для любой точки непрерывности $F_{\xi}(x)$ имеет место сходимость 
\begin{equation*}
