...
\subsection{Теорема Вейерштрасса о непрерывной функции на компакте}
\theorem Пусть $K\subset \R^n$ — компакт и функция $f: K\mapsto \R$ - непрерывная. Тогда $f$ на $K$ достигает наибольшее и наименьшее значения.
...
\begin{itemize}
    ...
    \begin{equation*}
        |x_i^k| \le \sqrt{\displaystyle\sum_{j=1}^n (x_j^k)^2} = x^k \le C \quad \text{для некоторого }C
    \end{equation*}
