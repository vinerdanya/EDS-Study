...
\definition Пусть $I$ — замкнутый, невырожденный брус и $\displaystyle\bigcup_{i=1}^kI_i = I$, где $I_i$ попарно не имеют общих внутренних точек. Тогда набор $\T = \{\T\}_{i=1}^k$ называется разбиением бруса $I$
...
\definition Масштаб разбиения $\T=\{I_i\}_{i=1}^k$ — число $\lambda(\T) = \Delta_{\T} = \displaystyle\max_{1\le i\le k} d(I_i)$
...
\definition Множество $M\subset\R^n$ будем называть \textit{множеством меры нуль по Лебегу}, если $\forall\ve>0$ существует не более чем счетный набор (замкнутых) брусов $\{I_i\}$ и выполняются:
\begin{enumerate}
...
\definition Функция $f:M\rightarrow\mathbb{R}$ \textit{непрерывна в точке} $x_0$, если
\begin{equation*}
...
\definition \textit{Верхним и нижним интегралом} Дарбу будем называть числа 
\begin{equation*}
...
\definition Пусть $D\subset\mathbb{R}^n$ — допустимое, $f:D\rightarrow\mathbb{R}$. Тогда, интегралом Римана $f$ по $D$ называется число $\mathcal{I}$:
\begin{equation*}
...
\subsection{[TBA] Свойства интегральных сумм Дарбу}
...
\subsection{[TBA] Утверждение о независимости определения допустимого множества от выбора бруса}
